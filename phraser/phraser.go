...
func init() {
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  $ %s --txtpath dumpz/textfiles --wikipath dumpz/mediawiki --omdbpath dumpz/omdb --ngrampath dumpz/ngram \n", os.Args[0])
...
var omdbFodderPath = flag.String("omdbpath", "", "Dir full of crawl_omdb files")
...
// readOMDB reads text files that crawl_omdb generated by crawling OMDB.
func readOMDB(fodderPath, tmpPath string) {
	lineCount := uint64(0)
	inFilePaths, _ := filepath.Glob(filepath.Join(fodderPath, "*.txt"))
	tally := counter{}
	for _, inFilePath := range inFilePaths {
		...
		for {
			...
			found.boost(title, uint64(score))
		}
		tallySnippets(&tally, found)
	}
	tallyPath := filepath.Join(tmpPath, fmt.Sprintf(tmpFilenameFormat, "omdb", lineCount))
	tally.persist(tallyPath)
}
...
func main() {
	...
	if *tmpPath == "" && *wikiFodderPath == "" && *ngramFodderPath == "" && *txtFodderPath == "" && *omdbFodderPath == "" {
		log.Fatal("none of --wikipath, --ngrampath, --txtpath, --omdbpath set. No input, nothing to do!")
	}
	if *tmpPath == "" {
		*tmpPath = filepath.Join(os.TempDir(), "phraser", nowPath)
		...
		if *omdbFodderPath != "" {
			readOMDB(*omdbFodderPath, *tmpPath) // read wikis, write tmp files
		}
		if *txtFodderPath != "" {
